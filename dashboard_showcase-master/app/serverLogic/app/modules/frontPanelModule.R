...
# start gauge modules ----
  Gauge1Data <- reactive({
    lstGaugeData <- sss(input$loan_amnt,input$term,input$installment,input$grade,input$sub_grade, input$Home_Ownership,input$annual_inc,
                        input$purpose, input$rate,input$emp)
    return(lstGaugeData)
  })
  ...
  #Gauge1 <- callModule(gaugeMod,350)
  Gauge1 <- callModule(gaugeModule, "Gauge1",title="Last 90 days",Gauge1Data)
